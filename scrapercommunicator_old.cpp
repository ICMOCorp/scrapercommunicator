...
//TODO
//This function shouldnt hang for this to work
...
int write_to_FIFO(WINDOW* window, int prow, int& fd, const char* msg){
    mvwprintw(window, prow++, 1, "Writing to FIFO: ");
    string msg_str(msg); 
    msg_str.resize(10);
    mvwprintw(window, prow++, 1, " %s...", msg_str.c_str());

    uint32_t lengthOfMsg = strlen(msg);
    ssize_t res = write(fd, &lengthOfMsg, sizeof(lengthOfMsg));
    if(res < 0){
        return -1;
    }

    size_t total_written = 0;
    while(total_written < lengthOfMsg){
        ssize_t bytes_written = write(fd, msg, strlen(msg));
        if(bytes_written < 0){
            mvwprintw(window, prow++, 1, "Error writing to FIFO");
            return -1;
        }
        total_written += bytes_written;
    }

    mvwprintw(window, prow++, 1, "Write Success!");
    return 1;
}
...
int connectToFIFO(WINDOW* window, int& IFIFO_opened, int& OFIFO_opened, struct pollfd* fds){
    ...
    if(ret < 0 ){
        mvwprintw(window, row++, 1, "Error in poll: ");
        mvwprintw(window, row++, 1, "  %s", strerror(errno));
        close(fds[0].fd);
        close(fds[1].fd);
        IFIFO_opened = false;
        ...
        mvwprintw(window, row++, 1, "Got nothin, must be empty");
        return 0;
    }
...
void communicator_job(WINDOW* initwindow, WINDOW* window, WINDOW* userwindow, WINDOW* backwindow){
    ...
    auto fifoCheckClockTick = std::chrono::high_resolution_clock::now();
    char lastCommandFromFIFO[10];
    memset(lastCommandFromFIFO, 0, 10);
    ...
    while (running_job) {
        ...
        //for fifo check we have to be sent a pulse
        int localfifostatus = connectToFIFO(backwindow, IFIFO_opened, OFIFO_opened, fds);
        if(localfifostatus == 1){
            fifoCheckClockTick = std::chrono::high_resolution_clock::now();
        }
        else if(localfifostatus == 0){
            std::chrono::duration<double> timePassedSinceFIFOCheck = 
                std::chrono::high_resolution_clock::now() - fifoCheckClockTick;
            if(timePassedSinceFIFOCheck > 10s){
                fifoCheckClockTick = std::chrono::high_resolution_clock::now();
                mvwprintw(window, windowRow++, 1, "FIFO empty too long   ");
                localfifostatus = -1;
                close(fds[0].fd);
                close(fds[1].fd);
                IFIFO_opened = false;
                ...
                mvwprintw(window, windowRow++, 1, "FIFO empty for %.3fs  ", 10 - timePassedSinceFIFOCheck.count());
            }
        }
        ...
        if(!connectedToASocketman){
            ...
            if(timePassedSincePing > 10s){
                ...
                mvwprintw(window, windowRow++, 1, "%.3f until next ping ", 10 - timePassedSincePing.count());
            }
        }
        ...
        if(localfifostatus > 0){
            ...
            mvwprintw(window, windowRow++, 1, "  >>%20s", lastCommandFromFIFO);
            wrefresh(backwindow);
        }
        else{
            mvwprintw(window, windowRow++, 1, "Fifo Status: X    ");
            mvwprintw(window, windowRow++, 1, "  %s", strerror(errno));
            wrefresh(backwindow);
        }
...
//====> ping handles both fifo and socket writing
        ...
        if(ret < 0){
            ...
            for(int i = 0;i<9;i++){
                lastCommandFromFIFO[i] = buffer[i];
            }
        }
...
int main(){
    ...
    */
    ...
    cout << " This may not be related but: " << endl;
    cout << " This is the final errorno string" << endl;
    cout << strerror(errno) << endl;
